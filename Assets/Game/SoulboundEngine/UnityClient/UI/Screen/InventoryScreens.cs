namespace SoulboundEngine.UnityClient.UI.Screen {
	...
	public class InventoryScreens {
		...
		public InventoryScreens() {
			Register(InventoryScreenHandlerType.PLAYER_INVENTORY, IProvider<PlayerInventoryScreenHandler, PlayerInventoryScreen>.Of(
				(ctx) => new PlayerInventoryScreen(ctx, AssetManager.Resolve<VisualTreeAsset>(new AssetKey("PlayerInventoryScreen")))
			));
		...
		public static IScreenHandle Open(InventoryScreenHandler handler, SoulboundUnityClient UnityClient, PlayerInventory playerInventory, PlayerEntity player) {
			try {
				ProviderContext ctx = CreateProviderContext(handler, playerInventory, player, UnityClient.ItemRenderManager);
				return UnityClient.OpenScreen(GetProvider(handler.GetHandlerType()).Create(ctx));
			} catch (Exception e) {
				Logger.LogFatal(e);
				return null;
			}
		...
		public static IScreenHandle Open<THandler>(InventoryScreenHandlerType<THandler> type, SoulboundUnityClient UnityClient, PlayerInventory playerInventory, PlayerEntity player) 
				where THandler : InventoryScreenHandler {
			try {
				ProviderContext ctx = CreateProviderContext(type.Create(playerInventory), playerInventory, player, UnityClient.ItemRenderManager);
				return UnityClient.OpenScreen(GetProvider(type).Create(ctx));
			} catch (Exception e) {
				Logger.LogFatal(e);
				return null;
			}
