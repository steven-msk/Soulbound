namespace SoulboundUnityEditor {
	...
	public class UXMLSchemaGenerator : AssetPostprocessor {
		...
		static void RegenerateAll() {
			...
			stringBuilder.AppendLine($"	using {nameof(SoulboundEngine.Registry)}");
			stringBuilder.AppendLine($"	using {nameof(UnityEngine.UIElements)}");
			stringBuilder.AppendLine();
			stringBuilder.AppendLine("    public static class UXMLSchema_Generated {");
			stringBuilder.AppendLine("        public static void RegisterAll() {");

			foreach (string path in allUXML) {
				...
				foreach (XElement element in document.Descendants().Where(e => e.Attribute("name") != null)) {
		...
		private static Type ResolveClrType(string uxmlTag) {
			...
			if (BUILT_IN_TYPES.TryGetValue(tag, out Type builtin)) return builtin;

			Type match = AppDomain.CurrentDomain.GetAssemblies()
