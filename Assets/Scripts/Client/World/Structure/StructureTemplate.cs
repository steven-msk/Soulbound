...
namespace SoulboundBackend.Client.World.Structure {
	public class StructureTemplate {
		...
		/// <param name="preliminaryData">The preliminary structure data generated from the placement function.</param>
		/// <returns>True or false depending on whether the data is valid for placement.</returns>
		public delegate bool PlacementValidationFunction(StructureGenerationContext context, PreliminaryStructureData preliminaryData);
		...
		private StructureTemplate(string ID, PlacementFunction placementFunction,
					PlacementValidationFunction validationFunction, PlacementGenerator placementGenerator,
		...
		public StructurePlacement FinalizePlacement(StructurePlacementConstraints placementConstraints) {
			ChunkBlockPos origin = placementConstraints.origin;
